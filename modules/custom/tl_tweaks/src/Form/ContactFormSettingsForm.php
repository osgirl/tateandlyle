...
class ContactFormSettingsForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    $config = \Drupal::configFactory()->get('tl_tweaks.settings');
    foreach (entity_get_bundles()['contact_message'] as $key => $contact_message) {
      if ($contact_message['label'] != 'Personal contact form') {
        $form['style'][$key] = array(
          ...
          '#size' => 75,
          '#maxlength' => 75,
          '#description' => $this->t('Specify the submit button text for the @form form.', array('@form' => $contact_message['label'])),
        );
      }
    }
    $form['submit'] = array(

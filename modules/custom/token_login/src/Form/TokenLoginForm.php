...
/**
 * @file
 * Contains \Drupal\token_login\Form\TokenLoginForm.
 */
...
class TokenLoginForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['actions']['submit'] = array('#type' => 'submit', '#value' => $this->t('Request login'));
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    ...
    $users = $this->userStorage->loadByProperties(array('mail' => $mail));
    $account = reset($users);
    ...
    if (!empty($account) && $this->validateEmail($reset_email)) {
      ...
      if (!empty($mail)) {
        $this->logger('user')->notice('Login instructions mailed to %name at %email.',
          array('%name' => $account->getUsername(), '%email' => $account->getEmail()));
      }

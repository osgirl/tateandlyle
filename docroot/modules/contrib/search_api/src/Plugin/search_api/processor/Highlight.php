...
class Highlight extends ProcessorPluginBase {
  ...
  protected function addExcerpts(array $results, array $keys) {
    ...
    foreach ($items as $item_id => $item) {
      ...
      foreach ($item as $field_id => $field) {
        // Do not consider the langcode as part of the snippet.
      ...
      $results[$item_id]->setExcerpt($this->createExcerpt(implode(" ", $text), $keys));
    }
  }
  ...
  protected function getFulltextFields(array $result_items, $load = TRUE) {
    ...
    // Include datasource-independent fields for all datasources.
    if (!empty($fulltext_fields[NULL])) {
      foreach (array_filter(array_keys($fulltext_fields)) as $datasource_id) {
        $fulltext_fields[$datasource_id] += $fulltext_fields[NULL];
      }
    }
    ...
    foreach ($result_items as $item_id => $result_item) {
      ...
      foreach ($fulltext_fields[$datasource_id] as $field_id => $field) {
        ...
        elseif ($load && $field->getDatasourceId()) {
  ...
  protected function createExcerpt($text, array $keys) {
    ...
    $excerpt = implode($ellipses[1], $out);

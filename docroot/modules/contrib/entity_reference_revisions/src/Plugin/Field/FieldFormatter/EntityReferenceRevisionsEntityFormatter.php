...
class EntityReferenceRevisionsEntityFormatter extends EntityReferenceRevisionsFormatterBase implements ContainerFactoryPluginInterface {
  ...
  public function settingsForm(array $form, FormStateInterface $form_state) {
    ...
    $elements['item_count'] = array(
      '#type' => 'textfield',
      '#title' => t('Item count'),
      '#default_value' => $this->getSetting('item_count'),
      '#description' => t('Number of referenced items to show. Set to 0, or empty to show all.')
    );
  ...
  public function settingsSummary() {
    ...
    $summary[] = t('@count items rendered as @mode', array(
      '@count' => $this->getSetting('item_count'),
  ...
  public function viewElements(FieldItemListInterface $items, $langcode) {
    $view_mode = $this->getSetting('view_mode');
    $item_count = $this->getSetting('item_count');
    $elements = array();

    $count = 1;
    foreach ($this->getEntitiesToView($items, $langcode) as $delta => $entity) {
      // Check how many items we have already listed and stop if we reached
      // the configured maximum.
      if ($item_count && $count > $item_count) {
        break;
      }
